...
def animate(
    ...
    rob_pos = {
        t: np.array(
            [
                [graph.nodes[traj[r, t]]["x"], graph.nodes[traj[r, t]]["y"]]
    ...
    conn_col = {
        (t, v1, v2): [] for t, conn_list in conn.items() for (v1, v2, b) in conn_list
    }
        ...
        for t in range(T + 1):
            ...
            for i, (v1, v2) in enumerate(graph.tran_edges()):
                if not node_explored[t, v1]:  # Hidden edge
                    tran_edge_alpha[t][i] = 0.0
                if (
                    node_explored[t, v1] and not node_explored[t, v2]
                ):  # Frontier node/edge
    ...
    coll_npos = ax.scatter(
        ...
        alpha=1,
        linewidths=1.0,
        edgecolors=np.full(len(graph.nodes), "black"),
    )
    # node labels
    coll_ntext = [
        ax.text(
    ...
    coll_tran_edge = nx.draw_networkx_edges(
        graph,
        dict_pos,
        ax=ax,
...
def animate_cluster(graph, traj, conn, subgraphs, dead_color="grey", **kwargs):
    ...
    clu_col = dict(
        zip(subgraphs, plt.cm.gist_rainbow(np.linspace(0, 1, len(subgraphs))))
    )
    node_colors = {
        (t, v): clu_col[c]
        for c, v_list in subgraphs.items()
        for v in v_list
        for t in range(T + 1)
    }
...
def animate_cluster_buildup(
    graph, problem, STEP_T=2, FPS=20, size=10, filename="clustering_animation.mp4"
):
    ...
    cluster_instances[len(cluster_instances)] = cluster_instances[
    ...
    coll_conn_edge = nx.draw_networkx_edges(
        graph, dict_pos, ax=ax, edgelist=list(graph.conn_edges()), edge_color="black"
    )
        ...
        for i, (v1, v2) in enumerate(graph.conn_edges()):
            if (v1, v2) in conn_activate[min(T, t)]:
                if anim_idx % (2 * freq) < freq:
                    ...
                    coll_conn_edge[i].set_linewidth(1.0)
                else:
