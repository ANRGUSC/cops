...
def animate_cluster_problem_sequence(graph, problem_list, ANIM_STEP=30, filename='animation.mp4', labels=False):
    ...
    # Build dictionary robot,time -> position
    traj_x = {(r,t): np.array([problem_list[0].graph.nodes[v]['x'], problem_list[0].graph.nodes[v]['y']])
    ...
    coll_npos = ax.scatter(npos[:,0], npos[:,1], s=350, marker='o',
                           c=ncolors, zorder=5, alpha=0.8,
                           linewidths=1.0, edgecolors=edge_ncolor)
    # node labels
    unknown_coll_ntext = [ax.text(npos[i,0], npos[i,1], str(n),
                         horizontalalignment='center',
                         verticalalignment='center',
                         zorder=5, size=8, color='grey',
                         family='sans-serif', weight='bold', alpha=1.0)
                         for i, n in enumerate(graph.nodes)
                         if n not in problem_list[0].graph.nodes]
    n_known = []
    known_coll_ntext = []
    ...
    # robot nodes
    pos = np.array([traj_x[(r, 0)] for r in graph.agents])
    colors = plt.cm.rainbow(np.linspace(0, 1, len(graph.agents)))
    coll_rpos = ax.scatter(pos[:,0], pos[:,1], s=140, marker='o',
                           c=colors, zorder=10, alpha=0.7,
                           linewidths=2, edgecolors='black')
    ...
    def animate(time_idx):
            ...
            for i, n in enumerate(graph.nodes):
                if n in problem.graph.nodes:
                    for j, c in enumerate(problem.subgraphs):
                        if n in problem.subgraphs[c]:
                            if c not in problem.subsinks:
                                ncolors[i] = node_colors[j]
                            else:
                                add = True
                                for r in problem.graph.agents:
                                    if n == problem.graph.agents[r]:
                                        for subs in problem.subsinks[c]:
                                            if r == subs:
                                                add = False
                                if add:
                                    ncolors[i] = node_colors[j]

            coll_npos.set_facecolors(ncolors)
        ...
        if isinstance(problem, ExplorationProblem):
            ...
            for i, n in enumerate(g.nodes):
                if g.nodes[n]['known']:
                    edge_ncolor[i] = 'k'
            coll_npos.set_edgecolors(edge_ncolor)
            ...
            # update known nodes
            x_known_new = [npos[i,0] for i, n in enumerate(graph.nodes)
            if n in g.nodes and g.nodes[n]['known']]
            y_known_new = [npos[i,1] for i, n in enumerate(graph.nodes)
            if n in g.nodes and g.nodes[n]['known']]
            n_known_new = [str(n) for i, n in enumerate(graph.nodes)
            if n in g.nodes and g.nodes[n]['known']]
            for i in range(len(n_known_new)):
                if n_known_new[i] not in n_known:
                    n_known.append(n_known_new[i])
                    known_coll_ntext.append(ax.text(x_known_new[i], y_known_new[i], n_known_new[i],
                                         horizontalalignment='center',
                                         verticalalignment='center',
